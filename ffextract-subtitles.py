    ...
    def get_ffmpeg_track_id(self, file_path):
        ...
        for stream in streams:
            ...
            if not fname_printed:
                action = "Scan" if self.args.scan_only else "Process"
            ...
            is_forced = (
                forced == 1 or "forc" in title.lower()
...
def main():
    ...
    parser.add_argument(
        "--get-sdh",
        action="store_true",
        help='get "Subtitles for the Dead and Hard of Hearing" (based on "SDH" in title attribute)',
    )
    parser.add_argument(
        "--get-forced",
        action="store_true",
        help='get "forced" subtitles (will use suffix "forced" in filename)',
    )
    parser.add_argument(
